...
从用户角度看，这会显得很奇怪：项目目录里明明已经有 `skills-lock.json` 记录了这个技能，但 `check` 和 `update` 却完全没有利用这份项目级锁文件。
...
而且 `runUpdate()` 在真正执行更新时，直接调用的是：

- `node <repo>/bin/cli.mjs add <installUrl> -g -y`
...
这说明当前 update 路径不仅“读取全局锁”，而且“执行全局重装”也是写死的。
...
### 3. 项目级锁文件也不保存“重建原安装形态”所需的信息

除了远端检查所需的 `sourceUrl` / `skillPath` / `skillFolderHash` 之外，项目级锁文件当前也没有保存：

- 安装到哪些 agent
- 使用的是 `copy` 还是 `symlink`

而项目级恢复/同步路径本身也证明了这一点：

- `experimental_install` 明确只往 `.agents/skills/`（universal agents）恢复
- `experimental_sync` 明确是 project-scoped，且固定使用 symlink
...
这意味着即使后续补齐了“远端版本检查元数据”，也仍然无法仅靠当前 `skills-lock.json` 忠实重建像 `--copy --agent claude-code` 这样的原安装语义。

### 4. 项目级锁文件没有接入 `check` / `update`
...
我建议先把需求拆成两层，不要混成一个“项目级更新支持”：

### 路线 A：先修作用域 / 提示问题（较小改动）

如果目标只是修复“在项目目录里执行 `skills check` / `skills update` 时行为不符合预期”，那么可以先做更小的一步：

1. 当存在 `skills-lock.json` 时，明确 `check/update` 的 project scope 语义
   - 要么只检查当前项目已跟踪的技能
   - 要么明确提示“项目级远端技能暂不支持 check/update”

2. 同步更新 README / help 文案
   - 明确 `skills check` / `skills update` 当前是 global-only，还是在什么条件下会切到 project scope

这条路线更接近 `#637`，解决的是“命令作用域”和“用户预期管理”问题。

### 路线 B：真正支持项目级远端技能更新（较大改动）

如果目标是让项目级 `skills-lock.json` 自己成为完整的更新来源，那至少还需要补两类信息：
...
2. 安装语义元数据
   - 安装目标 agent 列表
   - 安装模式：`copy` / `symlink`
   - 其他会影响重装结果的 project-scoped 信息

然后再为项目级增加单独的 `check/update` 路径：

1. 读取 `skills-lock.json`
2. 复用现有 GitHub folder hash 检查逻辑，或为 local / other source 设计对应比较方式
3. 更新时走 project-scoped reinstall，而不是 `-g`
4. 按锁文件记录的 agent / mode 重建安装结果

这条路线才是在解决“项目级锁文件是否应该成为真正的更新来源”这个设计问题。
...
- 一条是在修“scope bug / 预期管理”
- 一条是在做“项目级锁文件成为更新来源”的能力扩展

后续如果要正式实现，最好先统一“项目级锁文件到底是不是更新来源”这个设计，再决定 schema 要不要扩、要扩到什么程度。
